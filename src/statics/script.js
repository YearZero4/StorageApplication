...
contenido += `<div id="op"><div id="showinfo" data-sigil="${key}" onclick="copiarTexto(this)"><strong>${key}</strong><p> ${value}</div><img src="./src/icons/delete.png" id="delete" data-key="${key}"></div>`;
 }
...
const divs = document.querySelectorAll('#show > div');
divs.forEach(div => {
const sigil = div.querySelector('[data-sigil]').getAttribute('data-sigil').toLowerCase();
if (sigil.includes(searchTerm)) {
 div.style.display = '';
} else {
